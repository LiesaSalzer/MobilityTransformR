...
test_that("Transformation of numeric vectors works", {
  ...
  expect_equal(MobilityTransformR:::.transformNumeric(rtime, marker, tR = 0, 
                                                      U = 30, L = 90), transf)
  expect_error(mobilityTransform(rtime, marker[1,]), 
               "'U' and 'L' are expected to be of length 1.")
  expect_error(mobilityTransform(c(1, 2, 3, "a"), marker), 
               "'x' needs to be of class 'numeric', 'Spectra' or 'OnDistMSnExp' but 
          not class 'character'")
  expect_error(mobilityTransform(rtime), 
               "Missing data.frame 'marker' with marker information")
  expect_error(mobilityTransform(rtime, "A"), 
...
test_that("Transformation of Spectra works", {
  ...
  expect_equal(MobilityTransformR:::.transformSpectra(spectra_data, marker, tR = 0, 
                                                      U = 30, L = 90), transf)
  expect_error(mobilityTransform(spectra_data, marker[1,]), 
               "'U' and 'L' are expected to be of length 1.")
  expect_error(mobilityTransform("A", marker), 
               "'x' needs to be of class 'numeric', 'Spectra' or 'OnDistMSnExp' but 
          not class 'character'")
  expect_error(mobilityTransform(spectra_data), 
               "Missing data.frame 'marker' with marker information")
  expect_error(mobilityTransform(spectra_data, "A"), 
               "'marker' missing column 'rtime', 'mobility' or both")
...
test_that("Transformation of OnDiskExp works", {
  ...
  expect_equal(MobilityTransformR:::.transformOnDiskMSnExp(raw_data, marker, tR = 0, 
                                                      U = 30, L = 90), transf)
  expect_error(mobilityTransform(raw_data, marker[1,]), 
               "'U' and 'L' are expected to be of length 1.")
  expect_error(mobilityTransform("A", marker), 
               "'x' needs to be of class 'numeric', 'Spectra' or 'OnDistMSnExp' but 
          not class 'character'")
  expect_error(mobilityTransform(raw_data), 
               "Missing data.frame 'marker' with marker information")
  expect_error(mobilityTransform(raw_data, "A"), 
               "'marker' missing column 'rtime', 'mobility' or both")
  expect_error(mobilityTransform(raw_data, marker[,-4]), 
