...
However, the effective mobility of a compound remains stable in the same electrophoretic system. The use of an effective mobility scale instead of a migration time scale circumvents this drawback and will result in highly reproducible peaks, which has already been shown in 2001 @Schmitt-Kopplin2001. 
...
As a model for the <code>mobilityTransformationR</code> package, we have taken the functions from the recently developed ROMANCE software @ROMANCE2018. ROMANCE is an open-source software that transforms CE-MS data into an effective mobility scale. 
...
\begin{equation}
  ...
  (\#eq:mobility2)
\end{equation}
with $t\_A$, the MT of the secondary marker and its corresponding mobility $µ\_A$. 
